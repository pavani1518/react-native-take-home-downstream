...
- **"Build a React Native + TypeScript + Expo application from the attached requirements doc (`HIRING_REACT_NATIVE_TAKE_HOME.md`). Read the doc in detail and come up with a plan / step-by-step approach to implement and match the document requirements clearly before writing any code."** The initial directive. Forced a planning-first pass: I read the full 616-line spec, drew up a 16-phase execution plan organized by dependency (bootstrap → architecture → data → pure domain → tests → UI → native modules → offline → analytics → polish → docs), and surfaced 4 boundary decisions back to the user via `AskUserQuestion` (sheet library, §9 metadata choice, §10 offline choice, camera fallback). Only after the plan + answers were locked did I start running `create-expo-app`. This is the prompt that prevented "stream-of-consciousness coding" — every line written later traced back to a numbered phase in the plan.
- **"Make sure we are exactly following the requirements as per document. No hallucination, no guesses."** Follow-up sharpener before execution started. Reshaped the plan from "good-enough RN app" to "verbatim spec adherence". It led to an explicit pre-implementation audit of "things I'm REMOVING" vs "things I'm ADDING" against the doc, with verbatim spec citations. Examples cut: `date-fns`, `@gorhom/bottom-sheet`, toast notifications, optimistic rollback (Optional Stretch), expo-haptics, five actions (the doc says "at least three"). Examples added: exact `Missing proof / Scan mismatch / Ready to complete` filter labels, `Linking.openSettings()` recovery, sensor-unavailable state for the accelerometer.
...
- **"Audit the implementation against the requirements doc end-to-end. Goal: every functional requirement (§1–§14), UX rule, and architecture expectation in the spec must be verifiably met by the actual code. Steps: (1) build a verification matrix — list every claim from the doc, (2) map each claim to an executable check (grep / node script / file:line citation), (3) run the checks in priority order, (4) for any failure, fix it and re-verify. No hallucination, no guesses — every finding must cite a file:line or a re-runnable command."** Triggered the audit pass. The explicit "goal + steps + evidence rule" framing forced me into a verification-matrix approach instead of opportunistic grepping. I ran checks in priority order — domain purity first (cheapest), field-coverage next, numeric data shape, touch targets last (most likely to find a real bug). This caught the 3 spec violations (touch targets <44pt, missing `completed` workStatus, 3 unused deps), produced an audit report where every claim cites a file:line or a re-runnable command, and ended with each violation fixed + re-verified.
